...
public class BeanDeserializer extends DeserializerImpl implements Serializable {
            ...
            throws SAXException
    {
        ...
        if (typeDesc != null) {
            ...
            //If fileld name is null, one reason could be that Results element in soap
            //contains Derivedclass of BaseResult which can be either GetTaxResult,PostTaxResult... etc.
            // in that case we have to twik deseralization code to make it work
            if(fieldName==null && "ResultList".equals(javaType.getSimpleName()) ) {
                QName tempElemQName = new QName(namespace, "BaseResult");
                fieldName=typeDesc.getFieldNameForElement(tempElemQName,
                    isEncoded);
                propDesc = (BeanPropertyDescriptor)propertyMap.get(fieldName);
                fieldDesc = typeDesc.getFieldByName(fieldName);
                fieldDesc.setXmlType(elemQName);
                if(collectionIndex==-1)
                    collectionIndex=tempCollectionIndex;                
            }
